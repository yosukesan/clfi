...
#
    ...
    return '{0}-{1}'.format(year.group(2), *term)

def current_year(data: dict):
#
# return CurrentYTDuration assuming stored in the last element
#
    return int(list(data.values())[-1])
...
#
    ...
    ax.bar(df.index, df[target], alpha=0.5)
    ...
    ax2 = ax.twinx()
    ax2.plot(df[target].pct_change(), 'o-', color='red')
    ax2.set_ylabel('Yearly Change / %')

    i = 0
    for x, y in zip(df.index, df[target].pct_change()):
    ...
    plt.show()
    #plt.savefig('{0}.png'.format(firm[:-4]))
    plt.clf()
...
if __name__ == '__main__':
    ...
    res = {}
    sales_container : list = []
    time_stamp : list = []

    for file_time_stamp in d[firm]:
        if 'sr' in file_time_stamp:
            continue
        time_stamp.append(parse_file_time_stamp(file_time_stamp))
        sales_container.append(current_year(d[firm][file_time_stamp]['sales']))
